...
import com.leeahwi.book.springboot.config.auth.dto.OAuthAttributes;
import com.leeahwi.book.springboot.config.auth.dto.SessionUser;
import com.leeahwi.book.springboot.domain.user.User;
import com.leeahwi.book.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User>{
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        //지금은 구글만 사용 이후 네이버 로그인 연동시 네이버 로그인인지, 구글 로그인인지 구분하기 위해 사용
        ...
        //추후 네이버 등 다른 소셜 로그인도 이 클래스를 사용
        OAuthAttributes attributes = OAuthAttributes.of(registrationId, userNameAttributeName, oAuth2User.getAttributes());

        User user = saveOrUpdate(attributes);

        //SessionUser 세션에 사용자 정보를 저장하기 위한 Dto 클래스
        //User 클래스 왜 쓰지 않고 새롭게 만들어서 쓰는가
